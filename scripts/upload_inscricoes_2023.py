#!/usr/bin/env python3
"""
Script para processar e enviar inscrições de 2023 para o Supabase
"""
import pandas as pd
import requests
import unicodedata
...
def load_ibge_from_supabase():
    ...
    response = requests.get(
        f'{SUPABASE_URL}/rest/v1/ibge_municipios?select=*',
        headers=headers,
        params={'limit': 10000}
    )
    ...
    if response.status_code == 200:
        ...
        print(f"Erro ao carregar IBGE: {response.status_code}")
        return []
...
def map_row_to_inscricao(row, ano, ibge_data, moeda_original='USD'):
    ...
    if birthdate and pd.notna(birthdate):
        ...
        if pd.isna(birthdate):
            ...
            try:
                if isinstance(birthdate, str) and '-' in str(birthdate):
    ...
    inscricao = {
        ...
        'address': str(row.get('information_address', '')) if pd.notna(row.get('information_address')) else None,
...
def main():
    print("🚀 Iniciando processamento de inscrições de 2023...")
    ...
    existing_ids = load_existing_ids(2023)
    print(f"✅ Já existem {len(existing_ids)} registros no banco")
    
    # Ler arquivo Excel
    file_path = 'UTMB - 2023 - USD.xlsx'
