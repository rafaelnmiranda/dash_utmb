#!/usr/bin/env python3
"""
Script para processar e enviar inscrições de 2025 (BRL) para o Supabase
"""
import pandas as pd
import requests
import unicodedata
...
def map_row_to_inscricao(row, ano, ibge_data, moeda_original='BRL'):
    """Mapeia uma linha do Excel para o formato da tabela inscricoes"""
    
    # Corrigir cidade
    city_original = str(row.get('City', '')).strip()
    if not city_original or city_original == 'nan' or city_original == 'None':
        city_original = 'Não informado'  # Valor padrão se não tiver
    
    city_corrected = correct_city(city_original, ibge_data)
    if not city_corrected or city_corrected == 'nan' or city_corrected == 'None':
        city_corrected = city_original  # Usar original se correção falhar
    
    uf, regiao = get_ibge_info(city_corrected, ibge_data)
    
    # Normalizar valores monetários (já estão em BRL, não precisa converter)
    ...
    if registration_date:
        registration_date = pd.to_datetime(registration_date, errors='coerce', dayfirst=True)
    ...
    # Validar country (obrigatório)
    country = str(row.get('Country', '')).strip()
    if not country or country == 'nan' or country == 'None':
        # Tentar usar nationality como fallback
        country = str(row.get('Nationality', '')).strip()
        if not country or country == 'nan' or country == 'None':
            country = 'BR'  # Default para Brasil se não tiver
    
    # Mapear campos - 2025 tem 'Adress' (com um 'd' só)
    address_value = None
    if pd.notna(row.get('Adress')):
        address_value = str(row.get('Adress', ''))
    elif pd.notna(row.get('information_address')):
        address_value = str(row.get('information_address', ''))
    elif pd.notna(row.get('Address')):
        address_value = str(row.get('Address', ''))
...
def main():
    print("🚀 Iniciando processamento de inscrições de 2025 (BRL)...")
    ...
    existing_ids = load_existing_ids(2025)
    print(f"✅ Já existem {len(existing_ids)} registros no banco")
    
    # Ler arquivo Excel
    file_path = 'PARATY_BRAZIL_BY_UTMB__2025_ChatGPT_BRL 2025-10-03 02_51_05.xlsx'
